...
function ToastClose({ className, ...props }: ToastPrimitive.Close.Props) {
  return <ToastPrimitive.Close data-slot="toast-close" aria-label="Close toast" render={<Button variant="ghost" size="icon-sm" />} className={cn("relative shrink-0 text-muted-foreground hover:text-foreground", className)} {...props}><XIcon /></ToastPrimitive.Close>
}

function ToastIcon({ type }: { type: string | undefined }) {
  const icons = { success: CircleCheckIcon, info: InfoIcon, warning: TriangleAlertIcon, error: OctagonXIcon, loading: Loader2Icon }
  const Icon = type ? icons[type as keyof typeof icons] : undefined
  return Icon ? <span data-slot="toast-icon" className="shrink-0"><Icon className={cn("size-4", type === "loading" && "animate-spin", type === "error" && "text-destructive")} aria-hidden="true" /></span> : null
}

function ToastList() {
  const { toasts } = ToastPrimitive.useToastManager()
  return toasts.map((toastItem) => <Toast key={toastItem.id} toast={toastItem}><ToastContent><ToastIcon type={toastItem.type} /><div className="flex min-w-0 flex-1 flex-col gap-1"><ToastTitle /><ToastDescription /></div><ToastAction /><ToastClose /></ToastContent></Toast>)
}
